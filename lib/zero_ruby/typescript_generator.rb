  ...
  class TypeScriptGenerator
    ...
    def collect_input_objects_from_arguments(arguments)
      arguments.each do |_name, config|
        type = strip_wrappers(config[:type])

        # Then unwrap array types to check element type for InputObjects
        if array_type?(type)
          type = strip_wrappers(type.member)
        end
        ...
        if input_object_type?(type)
          ...
          unless @input_objects.key?(type_name)
            ...
            collect_input_objects_from_arguments(type.arguments_metadata)
          end
        end
      end
    end
    ...
    def resolve_type(type)
      ...
      if array_type?(type)
        return "#{resolve_type(type.member)}[]"
      end
